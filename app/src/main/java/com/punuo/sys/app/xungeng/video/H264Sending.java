...
import com.punuo.sys.app.xungeng.tools.AvcEncoder;
...
public class H264Sending extends Activity implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    private static final String TAG = "H264Sending";   //取得类名
    public static boolean G711Running = true;
    int frameSizeG711 = 160;
    private int previewFrameRate = 10;  //演示帧率
//    private final int previewWidth = 352;     //水平像素
//    private final int previewHeight = 288;     //垂直像素
    private static int previewWidth = 1280;     //水平像素
    private static int previewHeight = 720;     //垂直像素
...
//        SipInfo.sipDev.setResolution(this);
...
//        NativeH264Encoder.InitEncoder(previewWidth, previewHeight, previewFrameRate);
        avcEncoder=new AvcEncoder(previewWidth, previewHeight, previewFrameRate);
        SipInfo.flag=false;
    }
...
//                }
//                硬解码
                byte[] encodeResult = avcEncoder.offerEncoder(data, previewWidth, previewHeight); //进行编码，将编码结果存放进数组
...
//        avcEncoder.close();
//        NativeH264Encoder.DeinitEncoder();
//        Log.e(TAG, "onResolution: " +previewWidth+previewHeight+previewFrameRate);
//        changeResolution();
//        avcEncoder = new AvcEncoder(previewWidth, previewHeight, previewFrameRate);
//        SipURL sipURL = new SipURL("310023000100320160", SipInfo.serverIp, SipInfo.SERVER_PORT_USER);
//        SipInfo.toDev = new NameAddress("3245073", sipURL);
//        sipUser.sendMessage(SipMessageFactory.createNotifyRequest(sipUser, SipInfo.toDev,
//                SipInfo.user_from, BodyFactory.changeResolutionresponse(1)));
//    }

    private void changeResolution() {
        Log.e(TAG,"关闭摄像头" );
        mCamera.setPreviewCallback(null);//must do this，停止接收回调信号
        mCamera.stopPreview();//停止捕获和绘图
        Log.e(TAG,"关闭摄像头成功" );
    ...
    public void initCamera() throws IOException {
        Log.e(TAG, "初始化摄像头" );
        final Camera.Parameters parame = mCamera.getParameters();
        parame.setPreviewFrameRate(previewFrameRate);
        parame.setPreviewFormat(ImageFormat.YV12);
        Log.e(TAG,""+previewHeight+previewWidth  );
        parame.setPreviewSize(previewWidth, previewHeight);
